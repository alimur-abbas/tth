({
    doInit : function(component, event, helper) {
        const cols=[
            {label: 'Name', fieldName: 'Show_Account__c', type: 'url', sortable: true,typeAttributes: {label: { fieldName: 'Name' }, target: '_blank'}},
    ...
    navigateToSObjectRecord : function(component, event, helper) {
        ...
        if(selectedRows.length > 1){
            ...
            toastEvent.setParams({
                "title": "Sorry!!",
                "message": "You Need to select only one row at a time.",
                "type": "Error"
            });
            toastEvent.fire();
        }else{
